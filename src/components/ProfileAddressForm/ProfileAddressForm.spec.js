...
describe('ProfileAddressForm', () => {
  ...
  it('Renders with required props', () => {
    ...
    expect(wrapper.find('ProfileAddressForm').exists()).toBe(true);
  });
  ...
  it('Should fire onSubmit when form submitted with required fields', () => {
    ...
    const address1Input = wrapper.find('input[name="address1"]');
  ...
  it('Should update the state to match input values for all fields', () => {
    ...
    const address2Input = wrapper.find('input[name="address2"]');
    address2Input.prop('onChange')({ target: { value: '10 Testfield Road' } });

    const address3Input = wrapper.find('input[name="address3"]');
    ...
    countryInput.prop('onChange')({ label: 'Algeria', code: 'DZ', value: 4 });
